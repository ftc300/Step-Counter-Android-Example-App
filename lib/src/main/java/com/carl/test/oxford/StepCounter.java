...
public class StepCounter {
    ...
    public void start(){
        ...
        if (!active) {
            ...
            postProcessStage = new PostProcessStage(peakData, newStepCallback, eodCallback);
            new Thread(postProcessStage).start();
        }
    }
    ...
    public void processSample(String fileName,List<Axis> list) {
        start();
        if (active) {
            float magnitude = 0;
            for (int i = 0; i < list.size(); i++) {
                magnitude = list.get(i).x * list.get(i).x + list.get(i).y * list.get(i).y + list.get(i).z * list.get(i).z;
                magnitude = (float) Math.sqrt(magnitude);
                rawData.add(new DataPoint(i * 40 * 1000000, magnitude/100));
                if(i == list.size() - 1){
                    stop();
                    System.out.println(fileName+":"+postProcessStage.totalSteps);
                }
            }
        }
    }
