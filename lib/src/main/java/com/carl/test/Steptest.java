...
import com.carl.test.inshow.NewStepDetector;
import com.carl.test.oxford.StepCounter;
...
public class Steptest {
    public static void main(String[] args) {
        ...
        csv.add("yw_run126");
        csv.add("yw_slow154");
        System.out.print(csv.remove(0));
        for (String csvFile : csv) {
            List<Axis> list = readCsvFile(folderPath + csvFile + ".csv", "gb2312", ",");
            StepDetector stepCounter = new StepDetector(list,csvFile);
            stepCounter.onSensorChanged();
        }
    }
}
